...
class Oohlalog::Logger
  ...
  def initialize(buffer_size=100, level = DEBUG)
  ...
  def add(severity, message, category=nil, details=nil)
    ...
    if severity >= self.level
      @buffer << {level: severity_string(severity), message: message.gsub(/\e\[(\d+)m/, '') , category: category, details: details, timestamp:Time.now.to_i * 1000}
      check_buffer_size
    end
  end
...
private
  def send_payload(payload)
    begin
      request = Net::HTTP::Post.new("#{Oohlalog.path}?apiKey=#{Oohlalog.api_key}",{'Content-Type' =>'application/json'})
